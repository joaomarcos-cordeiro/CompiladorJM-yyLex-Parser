## Compiler
Desenvolvimento de um alfabeto próprio de linguagem, a partir da criação do compilador capaz de processar essa linguagem e gerar a Árvore de Sintaxe Abstrata (AST). Foram feitos testes que validam seu funcionamento, além de arquivos contendo erros propositais para verificar a detecção e o tratamento adequado de falhas - UFJ

## ~~~English
Development of a custom language alphabet, based on the creation of a compiler capable of processing this language and generating the Abstract Syntax Tree (AST). Tests were conducted to validate its functionality, along with files containing intentional errors to verify proper fault detection and handling — UFJ.


